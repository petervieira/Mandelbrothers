...
from sprites import *
from os import path # cuts out the need for 'os.' in 'os.path'
from mapp import *
from minimap import *
...
class Game:
    def __init__(self):
        ...
        self.textboxDelay = 0
        self.alpha = 0
        self.temp = pg.Surface((1024, 768))
        self.gamefolder = path.dirname(sys.executable)
        self.load_data()
    ...
    def load_data(self):
        if getattr(sys, 'frozen', False):
            ...
            self.gameFolder = path.dirname(path.realpath(__file__)) # __file__ standard variable is full path to current script

        pg.mixer.music.load(path.join(self.gameFolder, 'music/theme.wav'))
        pg.mixer.music.set_volume(.5)
        pg.mixer.music.play(-1, 0.0)
        self.map = TiledMap(path.join(self.gameFolder, 'maps/overworld.tmx'))
    ...
    def load_sprites(self, sprites):
        for name in sprites:
            self.sprites[name] = pg.image.load(path.join(self.gameFolder, 'images/') + name + '.png').convert_alpha()

    def load_sprites_scaled(self, images):
        for (name, w, h) in images:
            self.sprites[name] = pg.transform.scale(pg.image.load(path.join(self.gameFolder, 'images/') + name + '.png').convert_alpha(), (w, h))

    def load_sounds(self, sounds):
        self.sounds = dict([(name, pg.mixer.Sound(path.join(self.gameFolder, 'sounds/') + name + '.wav')) for name in sounds])
    ...
    def drawScreen(self):
        ...
        self.screen.blit(self.temp, (0,0))
        ...
        for event in pg.event.get():
            if event.type == pg.QUIT:
                ...
                if event.key == pg.K_ESCAPE:
                    ...
                    if self.on_main_menu:
                        pg.mixer.music.load(path.join(self.gameFolder, 'music/theme2.wav'))
                        pg.mixer.music.set_volume(.5)
                        pg.mixer.music.play(-1, 0.0)
                        self.newGame()
